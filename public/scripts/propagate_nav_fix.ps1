...
foreach ($file in $files) {
    ...
    # 1. Replace <nav>...</nav> block with CLEAN content
    # This wipes out the corrupted version from previous run
    if ($content -match "<nav[\s\S]*?</nav>") {
        ...
        if ($mappings.ContainsKey($file.Name)) {
            ...
            # Replace: class="btn-holo-forge nav-active"><span>LinkText</span>
